...
class RegisterUserCampaignView(APIView):
    ...
    def get_object(self, uname):
        try:
            reg_obj = RegisterUserCampaign.objects.filter(register__uname=uname)[0]
            return reg_obj
        except:
            return None
    ...
    def get(self, request, uname, format=None):
        ...
        instance = self.get_object(uname)
        if instance != None:
...
class GetAccountDetailAPIView(APIView):
    ...
    demo_url="https://demo-six.divsolution.com/TradersDetails"
    

    def account_detail_api(self, dict_data=None):
        
        if dict_data:
            if dict_data.get("server")=="Real":
                payload = json.dumps({
                    "login": dict_data.get("account")
                })
                headers = {
                ...
                # print("============Real==",json.loads(response.text))
                ...
                responses = requests.request("GET", self.demo_url, headers=headers, data=payload)
                # encoded_return_data = "request_message={0}".format(encode_jwt(data=dict_data))
                # response_api = requests.post(self.url, data=encoded_return_data, verify=False)
                print("============Demo==",json.loads(responses.text))
...
class UserLiveAccountAPIView(APIView):
    ...
    authentication_classes = (SessionAuthentication,)
    user_model = User
    live_account_api_url = "https://live-hme.divsolution.com/TradersDetails"
    ...
    def request_send(self, jwt_dict_data=None, url=None):
        ...
        payload = json.dumps({
            "login": jwt_dict_data.get("account")
        })
        headers = {
            'Content-Type': 'application/json'
        }

        response = requests.request("GET", self.live_account_api_url, headers=headers, data=payload)
        # encoded_return_data = "request_message={0}".format(encode_jwt(data=dict_data))
        # response_api = requests.post(self.url, data=encoded_return_data, verify=False)
...
class AccountPwdResetView(APIView):
    ...
    def account_reset_pwd(self, account=None, type=None):
        ...
        print('----reset pwd of client', jwt_dict_data['account'], '|pwd|', jwt_dict_data['password'])
        if not jwt_dict_data['account']:
            return {'status': 0, 'msg': HttpResponse('Your instance not exist in LiveAccount instance.')}
        # your password data
