...
class UserSignupTemplateView(TemplateView):
    ...
    #     return c_data
    def c_trader_account(self,email):
        url = "https://demo-hme.divsolution.com/registration"
    ...
    def post(self, request, *args, **kwargs):
        ...
        cid,response=self.c_trader_account(email_var)
        if response.status_code==400:
            
            messages.add_message(request, messages.ERROR,cid.get("error").get("message"))
            ...
            register = Register(fname=fname, client_id=last_user_client_id,c_id=cid.get("userId"), lname=lname,uname=uname,dob=dob,email=email,mob=mob, country_id=country,pwd1=pwd1,pwd2=pwd2,user_id=user_obj.id)
            ...
            camp_data_api = requests.get(f'https://ib-admin.hme158.com/api/v1/get-campaign-list').json()
            ...
            body =  "感谢您的注册， 您的个人登录信息如下；\n\n"\
                    "用户 ID: \t{}".format(last_user_client_id)+'\n'+\
                    "用户名: \t{}".format(uname)+'\n'+\
                    "用户密码: \t\t{}".format(pwd1)+'\n\n'+\
                    "每次您想登录和管理您的交易账户时，您都需要这些登录信息，因此请将它们妥善安全的保管，不要将它们提供给任何人以免造成损失。 \n"\
                    "登录您的客户端 www.6imarkets.com\n\n\n\n\n\n"\
                    "顺祝商祺!\n"\
                    "6iMarkets Team"
...
def VolumeCumulativeData(request):
    ...
    for each_ib in ib_ids_list:
        ...
        for each_robj in reg_user_camp_obj:
            print('each objects checking---------------',each_robj)
            ...
            #     print('--------exception as error----------', e)
                return JsonResponse({'msg':'No user found with this login'}, status=400)
...
class Blogs(View):
    ...
    context={'blog':blog,'blogcategory':blogcategory}
    def get(self, request):
...
class BlogsCategorys(View):
    
    template_name="webapp/blogcategory.html"
    
    def get(self, request,slug):
        
        blog=Blog.objects.filter(blog_category__slug=slug)
        blogcategory=BlogCategory.objects.all()
        context={'blog':blog,'blogcategory':blogcategory}
        return render(request,self.template_name,context)

class BlogDetail(View):
    template_name="webapp/blogdetail.html"
...
class Deposits(View):
    template_name="webapp/deposits.html"
   
    def get(self, request):
        return render(request,self.template_name)

class Withdrawal(View):
    template_name="webapp/withdrawal.html"
   
    def get(self, request):
        return render(request,self.template_name)

class Getintouch(View):
    template_name="webapp/getintouch.html"
...
class MT4(View):
    template_name="webapp/mt4.html"
   
    def get(self, request):
        return render(request,self.template_name)

class AccountType(View):
    template_name="webapp/accounttype.html"
