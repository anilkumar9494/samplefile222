...
class ClientMamabersListAPIView(ListAPIView):
    ...
    def list(self, *args, **kwargs):
        ...
        client_id = self.kwargs.get('client_id')
        q = self.get_queryset().filter(ref_code=client_id)
        if q:
            return Response(self.serializer_class(q, many=True).data, status=200)
        else:
            return Response(status=400)


class ClientMamabersListDictAPIView(APIView):
    ...
    def get_object(self, client_id):
        try:
            reg_obj = RegisterUserCampaign.objects.filter(ref_code=client_id)[0]
            return reg_obj
        except:
            return None
...
class ClientMemberCountListAPIView(ListAPIView):
    ...
    def list(self, *args, **kwargs):
        ...
        q = self.get_queryset().filter(ref_code=self.kwargs.get('client_id')).count()
...
def update_client_campaign(request, data):
    ...
    camp_code = ''
    ib_id, campaign_name, login = data.split('_')

    url_1 = f'https://ib-admin.hme158.com.cn/api/v1/get-campaign-list'
    json_check = requests.get(url_1).json()

    for data in json_check:
        if campaign_name == data['campaign_name'].strip():
            camp_code = data['campaign_code']

    reg_camp_obj = RegisterUserCampaign.objects.get(ref_code=ib_id, status=1, mt4_id=login)

    reg_camp_obj.campaign = campaign_name
    reg_camp_obj.campaign_code = camp_code
...
class RegisterUserCampaignEditApiView(APIView):

    authentication_classes = ()
    permission_classes = (AllowAny,)
    serializer_class = RegisterUserCampaignEditSerializer

    def get_object(self, reg_id):
        try:
            return RegisterUserCampaign.objects.get(register=reg_id)
        except:
            return None

    def get(self, request, reg_id):

        camp_obj = self.get_object(reg_id)
        serializer = self.serializer_class(camp_obj)
        return Response(serializer.data, status=200)

    def put(self, request, reg_id):
        
        camp_obj = self.get_object(reg_id)
        serializer = self.serializer_class(camp_obj, data=request.data)
